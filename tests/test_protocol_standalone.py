"""
Unit tests for IO Board protocol encoding and decoding (현행 API 기준).

Run: pytest tests/test_protocol_standalone.py

Tests the binary protocol implementation including message building,
parsing, checksum calculation, and error handling.

현행 모듈 경로는 services.io_board.protocol이고, MCDC(deadbolt control)의
DATA 필드명은 DEADBOLT다 (request: OPEN/CLOSE, response: UNLOCK/LOCKED).
checksum은 STX 다음 바이트부터 ETX까지(ETX 포함)의 XOR이다.
"""

import pytest

# src/ import 경로는 pyproject.toml [tool.pytest.ini_options] pythonpath가 제공한다.
from services.io_board.protocol import (
    ETX,
    STX,
    build_request,
    calculate_checksum,
...
class TestRequestBuilding:
    ...
    def test_build_request_checksum_covers_etx(self):
        """Checksum is XOR of everything after STX, ETX inclusive."""
        message = build_request("MC", "PD", {})
        expected = calculate_checksum(message[1:-1])  # CMD+SUBCMD+ETX
        assert message[-1] == expected

    def test_build_deadbolt_control_open_request(self):
        """Test building deadbolt control OPEN request."""
        message = build_request("MC", "DC", {"DEADBOLT": ord("O")})
    ...
    def test_build_deadbolt_control_close_request(self):
        """Test building deadbolt control CLOSE request."""
        message = build_request("MC", "DC", {"DEADBOLT": ord("C")})
...
class TestResponseParsing:
    ...
    def test_parse_deadbolt_control_response_unlock(self):
        """Test parsing deadbolt control response (state UNLOCK)."""
        data = b"MCDC" + b"O"
        checksum = calculate_checksum(data + ETX)
        message = STX + data + ETX + bytes([checksum])
        ...
        assert response.DATA.DEADBOLT == "UNLOCK"

    def test_parse_deadbolt_control_response_locked(self):
        """Test parsing deadbolt control response (state LOCKED)."""
        data = b"MCDC" + b"C"
        checksum = calculate_checksum(data + ETX)
        message = STX + data + ETX + bytes([checksum])
        ...
        assert response.COMMAND == "MC"
        assert response.SUBCOMMAND == "DC"
        assert response.DATA.DEADBOLT == "LOCKED"
...
class TestRoundTrip:
    ...
    def test_deadbolt_control_round_trip(self):
        """Test deadbolt control command round trip."""
        # Build OPEN request
        request = build_request("MC", "DC", {"DEADBOLT": ord("O")})
        assert b"O" in request

        # Simulate device responding with UNLOCK state
        data = b"MCDCO"
        checksum = calculate_checksum(data + ETX)
        response_msg = STX + data + ETX + bytes([checksum])
